...
const Menu = () => {
  ...
  const filtrarProductosPorPreferencias = (productos, preferencias) => {
    ...
    if (preferencias.favorito && preferencias.favorito !== 'todos') {
      filtrados = filtrados.filter(producto => {
        ...
        switch(preferencias.favorito) {
          ...
          case 'vegetariano':
            return nombreLower.includes('vegetariano') || nombreLower.includes('veggie') || 
                   nombreLower.includes('avocado') || nombreLower.includes('aguacate') ||
                   nombreLower.includes('pepino') || nombreLower.includes('cucumber');
          default:
            return true;
        }
      });
    ...
    // Filtrar por nivel de picante (si tus productos tienen campo de picante)
    if (preferencias.picante) {
      // Esto depende de si tus productos tienen un campo de nivel de picante
      // Si no tienen, puedes omitir este filtro o agregar el campo a tus productos
      if (preferencias.picante === 'suave') {
        filtrados = filtrados.filter(producto => {
          // Asume que los productos sin mención de picante son suaves
          return !producto.nombre.toLowerCase().includes('picante') && 
  ...
  return (
    <div className={styles.menu}>
      ...
      <h1>{activeFilter === 'recomendados' ? 'Recomendados para ti' : 'Nuestro Menú'}</h1>
